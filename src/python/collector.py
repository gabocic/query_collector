...
twengine_url = 'https://twbe7-dot-diesel-zenith-175500.uc.r.appspot.com/'
...
def rest_api_call(method,relpath,data):
    try:
        if method == 'POST':
            response = requests.post(twengine_url+relpath, data = data)

        if method == 'PUT':
            response = requests.put(twengine_url+relpath, data = data)
    except:
...
def collect_query_data(conn,sqltext,default_schema) :

    #default_schema = 'airportdb'
    #conn.query('USE '+default_schema)
    #r = conn.store_result()
    ...
    data = {"owner": 1}
    optjob_res = rest_api_call('POST','optjobs/',data)
    ...
    query_res = rest_api_call('POST','optjobs/'+optjob_id.__str__()+'/query/',data)
    if len(query_res.keys()) == 0:
        return optjob_id

    # Attach execution plan to job
    data = {"plan": json.dumps(explainjs)}
    qp_res = rest_api_call('POST','optjobs/'+optjob_id.__str__()+'/executionplan/',data)
    ...
    dbs_res = rest_api_call('POST','optjobs/'+optjob_id.__str__()+'/dbserver/',data)
    ...
    for tablename in tablesdict.keys():
        ...
        "index_length":table_row['INDEX_LENGTH'],"collation":table_row['TABLE_COLLATION'],"columns":json.dumps(columnsjson) }

        table_res = rest_api_call('POST','optjobs/'+optjob_id.__str__()+'/tables/',data)
        ...
        while True:
            ...
            index_res = rest_api_call('POST','optjobs/'+optjob_id.__str__()+'/tables/'+table_id.__str__()+'/indexes/',data)
...
class OptJob:
    def __init__(self,hostname,user,passwd,defdb,port,sqltext):
        ...
        try:
            ...
            self.job_id = collect_query_data(self.connection,sqltext,self.params['database'])
    ...
    def analyze(self):
        if self.job_id == None:
            ...
            query_res = rest_api_call('PUT','optjobs/'+self.job_id.__str__()+'/',{})
